...
let HuksSignVerifyWorker = {
  HuksKeySIZE1024: {
    tag: params.HksTag.HKS_TAG_KEY_SIZE,
    value: params.HksKeySize.HKS_DSA_KEY_SIZE_1024,
  },
  HuksKeyAlgDSA: {
  ...
  try {
    ...
    if (isSING) {
      HuksOptions.properties.splice(
        1,
        1,
        HuksSignVerifyWorker.HuksKeyRSAPurposeSINGVERIFY
      );
      HuksOptions.properties.splice(2, 0, HuksSignVerifyWorker.HuksKeySIZE1024);
      await publicGenerateKeyFunc(keyAlias, HuksOptions);
      HuksOptions.properties.splice(
        1,
        1,
        HuksSignVerifyWorker.HuksKeyDSAPurposeSIGN
      );
    ...
    if (isSING && thirdInderfaceName == 'abort') {
      ...
      HuksOptions.properties.splice(
        1,
        1,
        HuksSignVerifyWorker.HuksKeyDSAPurposeVERIFY
      );
...
parentPort.onmessage = async function (e) {
  ...
  let newSrcKeyAlies = e.data + 'new_01';
  let HuksOptions = {
    properties: new Array(
      HuksSignVerifyWorker.HuksKeyAlgDSA,
      HuksSignVerifyWorker.HuksKeyDSAPurposeSIGN,
      HuksSignVerifyWorker.HuksTagDSADigestSHA1
    ),
    inData: srcData63Kb,
  };
  ...
  HuksOptions = {
    properties: new Array(
      ...
      HuksSignVerifyWorker.HuksKeyDSAPurposeVERIFY,
      HuksSignVerifyWorker.HuksTagDSADigestSHA1,
      HuksSignVerifyWorker.HuksTagDSACOMMONSIZE
    ),
    inData: srcData63Kb,
  };
  await publicSignVerifyFunc(
