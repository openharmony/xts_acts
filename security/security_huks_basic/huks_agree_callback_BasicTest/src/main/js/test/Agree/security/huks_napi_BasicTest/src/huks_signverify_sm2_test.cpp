        ...
        {
            {
                ...
                .uint32Param = OH_HUKS_ALG_SM2
            }, {
                .tag = OH_HUKS_TAG_KEY_SIZE,
                .uint32Param = OH_HUKS_SM2_KEY_SIZE_256
            }, {
        ...
        {
            {
                ...
                .tag = OH_HUKS_TAG_KEY_SIZE,
                .uint32Param = OH_HUKS_AES_KEY_SIZE_512
            }, {
                .tag = OH_HUKS_TAG_PURPOSE,
                .uint32Param = OH_HUKS_KEY_PURPOSE_SIGN | OH_HUKS_KEY_PURPOSE_VERIFY
            }, {
...
void static FreeBuffAndDeleteKey(struct OH_Huks_ParamSet **paramSet1, struct OH_Huks_ParamSet **paramSet2,
    struct OH_Huks_ParamSet **paramSet3, const struct OH_Huks_Blob *keyAlias1, const struct OH_Huks_Blob *keyAlias2)
{
    ...
    const uint32_t signIndex, const uint32_t verifyIndex, const bool isTestSignData)
{
    ...
    do {
        ...
        if (ret.errorCode != (int32_t)OH_HUKS_SUCCESS) {
            ret.errorCode = ((ret.errorCode == g_genParamsTest[genIndex].result) ? (int32_t)OH_HUKS_SUCCESS : ret.errorCode);
            break;
        }
        ...
        if (ret.errorCode != (int32_t)OH_HUKS_SUCCESS) {
            ret.errorCode = ((ret.errorCode == g_signParamsTest[signIndex].result) ? (int32_t)OH_HUKS_SUCCESS : ret.errorCode);
            break;
        }
        ...
        if (ret.errorCode != (int32_t)OH_HUKS_SUCCESS) {
            ret.errorCode = ((ret.errorCode == g_verifyParamsTest[verifyIndex].result) ? (int32_t)OH_HUKS_SUCCESS : ret.errorCode);
            break;
        }
...
HWTEST_F(HuksSignVerifySM2Test, Security_HUKS_NAPI_SignVerify_SM2_0100, TestSize.Level0)
{
    ...
    OH_Huks_Result ret = HksSm2SignVerifyTestRun(&keyAlias, SUCCESS_RETURN_INDEX,
