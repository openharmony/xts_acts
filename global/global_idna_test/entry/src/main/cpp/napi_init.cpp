...
static napi_value testUbrk_setUText_0100(napi_env env, napi_callback_info info) {
    ...
    bb = ubrk_open(UBRK_WORD, 'zh_CN', NULL, 0, &icu_status);
...
static napi_value testUbrk_clone_0100(napi_env env, napi_callback_info info) {
    ...
    UErrorCode status = U_ZERO_ERROR;
    int32_t pos;
    someUterators[0] = ubrk_open(UBRK_WORD, 'en_US', text, u_strlen(text), &status);
...
static napi_value testUbrk_clone_0200(napi_env env, napi_callback_info info) {
    ...
    UBreakIterator *brk3;
    UErrorCode status = U_ZERO_ERROR;
    int32_t pos, pos2, pos3;
    someUterators[0] = ubrk_open(UBRK_WORD, 'en_US', text, u_strlen(text), &status);
    brk = ubrk_clone(someUterators[0], &status);
    pos = ubrk_next(brk);
    OH_LOG_ERROR(LOG_APP, 'TestNdk pos is %{public}d', pos);
    brk2 = ubrk_clone(brk, &status);
    pos2 = ubrk_next(brk2);
    OH_LOG_ERROR(LOG_APP, 'TestNdk pos is %{public}d', pos2);
    brk3 = ubrk_clone(brk2, &status);
    pos3 = ubrk_next(brk3);
    OH_LOG_ERROR(LOG_APP, 'TestNdk pos is %{public}d', pos3);
    bool flagA = (pos == 4);
    bool flagB = (pos2 == 4);
    bool flagC = (pos3 == 5);
