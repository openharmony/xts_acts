...
describe('ChildProcessTest', function () {
   ...
   it('testRunCmd001', 0, async function (done) {
       ...
       console.log("test start +++ 11111")
       child.wait()
       console.log("test start +++ 2222")
       var res = new Uint8Array([97, 98, 99, 10, 0])
       console.log("test start +++ 333")
       await child.getOutput().then(val=>{
           console.log("test_runCmd_00" + val)
           for (var i = 0; i < res.length; i++) {
               console.log("test then array [i]= " + res[i]);
               console.log("test then val [i]= " + val[i]);
               expect(val[i]).assertEqual(res[i])
           }
       })
       done();
   })
