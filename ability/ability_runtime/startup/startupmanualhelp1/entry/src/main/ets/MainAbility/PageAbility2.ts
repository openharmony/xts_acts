...
export default class PageAbility2 extends UIAbility {
  ...
  onCreate(want:Want, launchParam) {
    ...
    commonEventManager.createSubscriber(subscribeInfo).then(data => {
      ...
      console.info("StartupTest PageAbility2 createSubscriber failed, error=" + JSON.stringify(error));
    })
  }
  ...
  onWindowStageCreate(windowStage: window.WindowStage) {
    ...
    windowStage.loadContent('pages/index', (err, data) => {
      if (err.code) {
        hilog.error(0x0000, 'StartupTest PageAbility2 ', 'Failed to load the content. Cause: %{public}s',
          JSON.stringify(err) ?? '');
        return;
      }
      hilog.info(0x0000, 'StartupTest PageAbility2 ', 'Succeeded in loading the content. Data: %{public}s',
        JSON.stringify(data) ?? '');
    });
  }
  ...
  onForeground() {
    ...
    setTimeout(()=>{
      if (!customConfig) {
        console.log('StartupTest PageAbility2 startupManager run defaultConfig');
