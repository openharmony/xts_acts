...
export default class AbilityA extends UIAbility {
  ...
  onWindowStageCreate(windowStage: window.WindowStage) {
    ...
    //  ./pages/IndexA
    windowStage.loadContent('abilitya/pages/IndexA', (err, data) => {
      if (err.code) {
        hilog.error(0x0000, 'testTag AbilityA', 'Failed to load the content. Cause: %{public}s', JSON.stringify(err) ?? '');
        return;
      }
      hilog.info(0x0000, 'testTag AbilityA', 'Succeeded in loading the content. Data: %{public}s', JSON.stringify(data) ?? '');
    });
  }
