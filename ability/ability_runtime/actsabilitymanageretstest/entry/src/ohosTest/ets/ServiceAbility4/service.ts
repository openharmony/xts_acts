  ...
  onCommand(want, startId) {
    ...
    let options = {
      onConnect: async function (element: any, proxy: any) {
        console.log('particleAbility_connectAbility_test_0300 ConnectAbility onConnect element.deviceId : ' + JSON.stringify(element.deviceId))
    ...
    particleAbility.disconnectAbility(connection_succeeded)
      .then((data: any) => {
        ...
        console.info('particleAbility_connectAbility_test_0300 disconnectability succeeded:' + JSON.stringify(data));
      })
      .catch((error: BusinessError) => {
        console.error('particleAbility_connectAbility_test_0300 disconnectability failed. Cause: ' + JSON.stringify(error));
      })
  },
  onConnect(want) {
