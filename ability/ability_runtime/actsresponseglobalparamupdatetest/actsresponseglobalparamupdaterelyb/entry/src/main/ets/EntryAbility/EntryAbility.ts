...
export default class EntryAbility extends UIAbility {
  ...
  onConfigurationUpdate(newConfig: Configuration): void {
    ...
    commonEventManager.publish(
      'RelyB_Ability_ConfigurationUpdateEvent', publishData, (err: BusinessError) => {
      if (err) {
        hilog.info(0x0000, TAG,
          'RelyB_Ability_ConfigurationUpdateEvent publish failed, Cause: %{public}s', JSON.stringify(err) ?? '');
      } else {
        hilog.info(0x0000, TAG, '%{public}s', 'RelyB_Ability_ConfigurationUpdateEvent publish successful');
      }
  });
  }
}
