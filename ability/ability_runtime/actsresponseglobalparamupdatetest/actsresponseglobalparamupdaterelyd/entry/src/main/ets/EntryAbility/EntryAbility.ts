...
export default class EntryAbility extends UIAbility {
  ...
  onConfigurationUpdate(newConfig: Configuration): void {
    ...
    commonEventManager.publish(
      'RelyD_Ability_ConfigurationUpdateEvent', publishData, (err: BusinessError) => {
      if (err) {
        hilog.info(0x0000, TAG,
          'RelyD_Ability_ConfigurationUpdateEvent publish failed, Cause: %{public}s', JSON.stringify(err) ?? '');
      } else {
        hilog.info(0x0000, TAG, '%{public}s', 'RelyD_Ability_ConfigurationUpdateEvent publish successful');
      }
    });
  }
}
