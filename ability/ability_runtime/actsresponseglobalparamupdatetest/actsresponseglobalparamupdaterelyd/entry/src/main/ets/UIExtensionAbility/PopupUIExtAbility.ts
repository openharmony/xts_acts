...
const TAG: string = 'PopupUIExtAbilityD';
...
export default class PopupUIExtAbility extends UIExtensionAbility {
  ...
  onConfigurationUpdate(newConfig: Configuration): void {
    ...
    commonEventManager.publish(
      'RelyD_PopupUIExtAbility_ConfigurationUpdateEvent', publishData, (err) => {
      if (err) {
        hilog.info(0x0000, TAG,
          'RelyD_PopupUIExtAbility_ConfigurationUpdateEvent publish failed, Cause: %{public}s',
          JSON.stringify(err) ?? '');
      } else {
        hilog.info(0x0000, TAG, '%{public}s', 'RelyD_PopupUIExtAbility_ConfigurationUpdateEvent publish successful');
      }
    });
  }
}