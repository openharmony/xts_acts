...
const TAG: string = 'UIExtAbilityE';
...
export default class UIExtAbility extends UIExtensionAbility {
  ...
  onConfigurationUpdate(newConfig: Configuration): void {
    ...
    commonEventManager.publish(
      'RelyE_UIExtAbility_ConfigurationUpdateEvent', publishData, (err) => {
      if (err) {
        hilog.info(0x0000, TAG,
          'RelyE_UIExtAbility_ConfigurationUpdateEvent publish failed, Cause: %{public}s', JSON.stringify(err) ?? '');
      } else {
        hilog.info(0x0000, TAG, '%{public}s', 'RelyE_UIExtAbility_ConfigurationUpdateEvent publish successful');
      }
    });
  }
}