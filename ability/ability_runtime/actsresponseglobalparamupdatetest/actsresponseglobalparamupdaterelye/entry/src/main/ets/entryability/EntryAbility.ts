...
export default class EntryAbility extends UIAbility {
  ...
  onConfigurationUpdate(newConfig: Configuration): void {
    ...
    commonEventManager.publish(
      'RelyE_Ability_ConfigurationUpdateEvent', publishData, (err) => {
      if (err) {
        hilog.info(0x0000, TAG,
          'RelyE_Ability_ConfigurationUpdateEvent publish failed, Cause: %{public}s', JSON.stringify(err) ?? '');
      } else {
        hilog.info(0x0000, TAG, '%{public}s', 'RelyE_Ability_ConfigurationUpdateEvent publish successful');
      }
    });
  }
}
