...
export default class UiExtAbility extends ShareExtensionAbility {
  ...
  onSessionCreate(want, session) {
    ...
    if (want.action === 'StartAbilityForResultAsCaller_0700') {
      setTimeout(() => {
        this.context.startAbilityForResultAsCaller({
          ...
          action: 'StartAbilityForResultAsCaller_0700_2'
        }).then((data) => {
    ...
    if (want.action === 'StartAbilityForResultAsCaller_0900') {
      console.info('====>[backGroundAbility] onSessionCreate want.action == StartAbilityForResultAsCaller_0900');
      this.context.startAbilityForResultAsCaller({
        bundleName: 'com.example.uiextensionforresultascallerother',
        abilityName: 'CalledAbility',
        moduleName: 'entry',
      ...
      console.info('====>[backGroundAbility] onSessionCreate want.action == StartAbilityForResultAsCaller_1200');
      this.context.startAbilityForResultAsCaller({
        ...
        commonEvent.publish('1200', commonEventData, (err) => {
          ...
          console.debug('====>[backGroundAbility] 1200 publish err:' + JSON.stringify(err));
        })
      })
    }
  }
