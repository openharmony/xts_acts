...
let TAG = 'clearUpApplicationDataHelp';
let startType;
let createFile;
let notClear;
let cacheDirPath;
let filesDirPath;
let preferencesDirPath;
let tempDirPath;
let databaseDirPath;
export default class MainAbility extends Ability {
    async onCreate(want, launchParam) {
        ...
        cacheDirPath = context.cacheDir + filePathName;
        filesDirPath = context.filesDir + filePathName;
        preferencesDirPath = context.preferencesDir + filePathName;
        tempDirPath = context.tempDir + filePathName;
        databaseDirPath = context.databaseDir + filePathName;
        if (createFile) {
            ...
            try {
                ...
                if (fs.accessSync(cacheDirPath) && fs.accessSync(filesDirPath) && fs.accessSync(preferencesDirPath) && fs.accessSync(tempDirPath) && fs.accessSync(databaseDirPath)) {
                    ...
                    commonEventManager.publish('clearUpApplicationDataEvent', options, (err) => {
