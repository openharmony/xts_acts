...
export default class UIExtAbility15 extends UIExtensionAbility {
  onCreate(launchParam: AbilityConstant.LaunchParam) {
    console.info(TAG, `s15UIExtAbility onCreate _0100——0013` + JSON.stringify(launchParam));
    ...
    commonEvent.publish('0100', commonEventData, (err) => {
      console.info('====> [s15UIExtensionForResultAsCallerCalledAbility 0100——0017] [0100] publish：%{public}s',
        JSON.stringify(err));

      this.context.terminateSelf().catch((err) => {
        console.info('====> [s15UIExtensionForResultAsCallerCalledAbility] [0100——0018] terminateSelf：%{public}s',
          JSON.stringify(err));
      });
    });
  }
