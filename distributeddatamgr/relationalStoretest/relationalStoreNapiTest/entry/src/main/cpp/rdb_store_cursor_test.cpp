...
static napi_value SUB_DDM_RDB_CURSOR_0300(napi_env env, napi_callback_info info) {
    ...
    NAPI_ASSERT(env, strcmp(name, "data1") == 0, "getColumnName data1 is fail.");
    
    errCode = cursor->getColumnName(cursor, 2, name, 6);
    NAPI_ASSERT(env, strcmp(name, "data2") == 0, "getColumnName data2 is fail.");

    errCode = cursor->getColumnName(cursor, 3, name, 6);
    NAPI_ASSERT(env, strcmp(name, "data3") == 0, "getColumnName data3 is fail.");

    errCode = cursor->getColumnName(cursor, 4, name, 6);
    NAPI_ASSERT(env, strcmp(name, "data4") == 0, "getColumnName data4 is fail.");

    errCode = cursor->getColumnName(cursor, 5, name, 6);
    NAPI_ASSERT(env, strcmp(name, "data5") == 0, "getColumnName data5 is fail.");
...
static napi_value SUB_DDM_RDB_CURSOR_0400(napi_env env, napi_callback_info info) {
    ...
    NAPI_ASSERT(env, strcmp(data1Value, "zhangSan") == 0, "getText  is fail.");
...
static napi_value SUB_DDM_RDB_CURSOR_0800(napi_env env, napi_callback_info info) {
    ...
    NAPI_ASSERT(env, strcmp(name, "") == 0, "getColumnName is fail.");
    predicates->destroy(predicates);
    errCode = cursor->destroy(cursor);
...
static napi_value SUB_DDM_RDB_CURSOR_1300(napi_env env, napi_callback_info info) {
    ...
    NAPI_ASSERT(env, strcmp(reinterpret_cast<char*>(data4Value), "string") == 0, "getBlob is fail.");
...
static napi_value SUB_DDM_RDB_CURSOR_2200(napi_env env, napi_callback_info info) {
    ...
    NAPI_ASSERT(env, strcmp(name, "name") == 0, "OH_Data_Asset_GetName is fail.");
    char uri[10] = "";
    size_t uriLength = 10;
    errCode = OH_Data_Asset_GetUri(asset, uri, &uriLength);
    NAPI_ASSERT(env, strcmp(uri, "uri") == 0, "OH_Data_Asset_GetUri is fail.");
    char path[10] = "";
    size_t pathLength = 10;
    errCode = OH_Data_Asset_GetPath(asset, path, &pathLength);
    NAPI_ASSERT(env, strcmp(path, "path") == 0, "OH_Data_Asset_GetPath is fail.");
