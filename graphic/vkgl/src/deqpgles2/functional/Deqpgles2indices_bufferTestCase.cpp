...
static HWTEST_F(ActsDeqpgles20017TestSuite, TestCase_016338, Function | MediumTest | Level2)
{
    GTEST_LOG_(INFO) << "TestCase_016338 start";
    int argc = 3;
    const char *argv[3] = {
        ".",
        "--deqp-case="
    ...
    if (result.numNotSupported == 1) {
        GTEST_LOG_(INFO) << "TestCase_016338 notsupport!";
    } else if (result.isComplete) {
        EXPECT_TRUE(result.isComplete);
        EXPECT_TRUE(result.numPassed == 1);
    };
    GTEST_LOG_(INFO) << "TestCase_016338 end";
}

static HWTEST_F(ActsDeqpgles20017TestSuite, TestCase_016339, Function | MediumTest | Level2)
{
    GTEST_LOG_(INFO) << "TestCase_016339 start";
    int argc = 3;
    const char *argv[3] = {
        ".",
        "--deqp-case="
    ...
    ActsDeqpgles20017TestSuite::runResult.numWaived += result.numWaived;
    if (result.numNotSupported == 1) {
        GTEST_LOG_(INFO) << "TestCase_016339 notsupport!";
    } else if (result.isComplete) {
        EXPECT_TRUE(result.isComplete);
        EXPECT_TRUE(result.numPassed == 1);
    };
    GTEST_LOG_(INFO) << "TestCase_016339 end";
}
