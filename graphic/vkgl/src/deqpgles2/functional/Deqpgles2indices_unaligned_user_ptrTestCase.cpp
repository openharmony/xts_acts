...
static HWTEST_F(ActsDeqpgles20017TestSuite, TestCase_016337, Function | MediumTest | Level2)
{
    GTEST_LOG_(INFO) << "TestCase_016337 start";
    int argc = 3;
    const char *argv[3] = {
        ".",
        "--deqp-case="
        "dEQP-GLES2.functional.draw.draw_element"
    ...
    if (result.numNotSupported == 1) {
        GTEST_LOG_(INFO) << "TestCase_016337 notsupport!";
    } else if (result.isComplete) {
        EXPECT_TRUE(result.isComplete);
        EXPECT_TRUE(result.numPassed == 1);
    };
    GTEST_LOG_(INFO) << "TestCase_016337 end";
}
