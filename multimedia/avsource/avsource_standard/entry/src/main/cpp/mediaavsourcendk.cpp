...
{
    int checkParam = FAIL;
    char uri[] = {"https://gossv.cfp.cn/videos/mts_videos/medium/VCG42N1325062494.mp4"};
    OH_AVSource *backParam = nullptr;
    backParam = OH_AVSource_CreateWithURI(uri);
...
static napi_value AVSourceCreateWithFD(napi_env env, napi_callback_info info)
{
    ...
    char fileName[] = {"/data/storage/el2/base/files/Fzl.mp4"};
    int fd = open(fileName, O_CREAT, PARAM_0666);
...
static napi_value AVSourceGetTrackFormat(napi_env env, napi_callback_info info)
{
    ...
    getSource = OH_AVSource_CreateWithFD(fd, PARAM_0, fileSize);
    if (getSource != nullptr) {
        int32_t trackCount = PARAM_0;
        for (uint32_t index = PARAM_0; index < (static_cast<uint32_t>(trackCount)); index++) {
            checkParam = OH_AVSource_GetTrackFormat(getSource, index);
            if (checkParam != nullptr) {
                backParam = SUCCESS;
            }
        }
