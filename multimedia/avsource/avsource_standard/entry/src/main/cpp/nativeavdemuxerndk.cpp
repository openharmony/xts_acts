...
static OH_AVSource *GetSource()
{
    ...
    int fd = open("/data/storage/el2/base/files/demo3.mp4", O_RDONLY);
    int64_t fileSize = GetFileSize("/data/storage/el2/base/files/demo3.mp4");
    int offset = ZEROVAL;
...
static napi_value OHAVDemuxerReadSample(napi_env env, napi_callback_info info)
{
    ...
    uint32_t videoTrackIndex = ZEROVAL;
    demuxer = OH_AVDemuxer_CreateWithSource(GetSource());
    OH_AVMemory *buffer = OH_AVMemory_Create(ONEONEVAL);
    OH_AVDemuxer_SelectTrackByID(demuxer, audioTrackIndex);
    OH_AVDemuxer_SelectTrackByID(demuxer, videoTrackIndex);
    int32_t ret;
    ...
    if (!audioIsEnd) {
        ret = OH_AVDemuxer_ReadSample(demuxer, audioTrackIndex, buffer, &attrinfo);
        if (ret == AV_ERR_OK) {
            returnValue = SUCCESS;
        }
    }
