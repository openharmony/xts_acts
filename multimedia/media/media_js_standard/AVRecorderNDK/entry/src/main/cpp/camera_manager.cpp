...
#define LOG_TAG "MY_NDKDEMO_CAMERA"
...
NDKCamera::NDKCamera(uint32_t focusMode, uint32_t cameraDeviceIndex, uint32_t sceneMode, char *previewId, char *photoId,
    ...
    : focusMode_(focusMode), cameraDeviceIndex_(cameraDeviceIndex), previewSurfaceId_(previewId),
      ...
      sceneMode_(NORMAL_VIDEO), exposureMode_(EXPOSURE_MODE_LOCKED), ret_(CAMERA_OK), size_(0), minExposureBias_(0),
...
Camera_ErrorCode NDKCamera::CreateMetadataOutput(void)
{
    ...
    if (metadataOutput_ == nullptr || ret_ != CAMERA_OK) {
        OH_LOG_ERROR(LOG_APP, "OH_CameraManager_CreateMetadataOutput failed. %{public}d ", ret_);
        return CAMERA_INVALID_ARGUMENT;
    } else {
