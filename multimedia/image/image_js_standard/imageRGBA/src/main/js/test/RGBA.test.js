...
export default function imageRGBA() {
    describe("Image", function () {
        let globalpixelmap;
        const { RGBA_F16, BGRA_8888, ALPHA_8, RGB_565, RGBA_8888, UNKNOWN, RGB_888 } = image.PixelMapFormat;
        ...
        it("SUB_MULTIMEDIA_IMAGE_RGBA_CREATE_PIXELMAP_PROMISE_0500", 0, async function (done) {
            const Color = new ArrayBuffer(96);
            let opts = { editable: true, pixelFormat: RGBA_8888, size: { height: 4, width: 6 } };
            image
                .createPixelMap(Color, opts)
                .then((pixelmap) => {
                    globalpixelmap = pixelmap;
         ...
         * @tc.name      : create pixelmap-callback(editable: true, pixelFormat: RGBA_8888, size: { height: 6, width: -1 })
         * @tc.desc      : 1.create InitializationOptions object
         *                 2.set editable,pixeFormat,size
         *                 3.using color and opts create newPixelMap
         *                 4.return newpixelmap empty
        ...
        it("SUB_MULTIMEDIA_IMAGE_RGBA_CREATE_PIXELMAP_CALLBACK_0700", 0, async function (done) {
            const Color = new ArrayBuffer(96);
            let opts = { editable: true, pixelFormat: RGBA_8888, size: { height: 6, width: -1 } };
            image.createPixelMap(Color, opts, (err, pixelmap) => {
                if (err) {
