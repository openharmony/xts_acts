...
static ArkUI_NodeHandle CreateChildNode(ArkUI_NativeNodeAPI_1 *nodeAPI)
{
    ...
    value[0].u32 = 0xFF000000;  // 0xFF000000
    nodeAPI->setAttribute(column1, NODE_BACKGROUND_COLOR, &item);
    value[0].u32 = 0xFFCCCCCC;
    ...
    nodeAPI->registerNodeEvent(bt1, NODE_ON_CLICK, 9, nullptr);
    ...
    nodeAPI->setAttribute(text1, NODE_BACKGROUND_COLOR, &item);
    value[0].u32 = 0xFF00FFFF;  // blue    
    nodeAPI->setAttribute(text2, NODE_BACKGROUND_COLOR, &item);
    value[0].u32 = 0xFF00FF00;  // green    
    nodeAPI->setAttribute(text3, NODE_BACKGROUND_COLOR, &item);
    value[0].u32 = 0xFF00FFFF;  // blue    
    nodeAPI->setAttribute(text4, NODE_BACKGROUND_COLOR, &item);
    
    value[0].f32 = 100;
    nodeAPI->setAttribute(text, NODE_WIDTH, &item);
    nodeAPI->setAttribute(text1, NODE_WIDTH, &item);
    nodeAPI->setAttribute(text2, NODE_WIDTH, &item);
    nodeAPI->setAttribute(text3, NODE_WIDTH, &item);
    nodeAPI->setAttribute(text4, NODE_WIDTH, &item);
    value[0].f32 = 30;
    nodeAPI->setAttribute(text, NODE_HEIGHT, &item);
    nodeAPI->setAttribute(text1, NODE_HEIGHT, &item);
    nodeAPI->setAttribute(text2, NODE_HEIGHT, &item);
    nodeAPI->setAttribute(text3, NODE_HEIGHT, &item);
    nodeAPI->setAttribute(text4, NODE_HEIGHT, &item);
...
napi_value NodeUtilMoveTest::testNodeUtilMove001(napi_env env, napi_callback_info info)
{
    ...
    nodeAPI->registerNodeEvent(column1, NODE_ON_CLICK, 1, a);
    nodeAPI->registerNodeEvent(column2, NODE_ON_CLICK, 2, a);
    ...
    nodeAPI->registerNodeEventReceiver([](ArkUI_NodeEvent *event) {
        ...
        if (targetId == 1) {
            ...
            if (strcmp(getParentId, "column2_id") == 0) {
                OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "Manager", "kkk666 getChild1 :%{public}s ", getParentId);
