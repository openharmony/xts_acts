...
napi_value StackOnAreaChangeTest::CreateNativeNode(napi_env env, napi_callback_info info)
{
    ...
    auto stackSpanSize = createChildNode(nodeAPI, COLOR_RED, ON_CLICK_EVENT_ID);
    auto stackSpanMove = createChildNode(nodeAPI, COLOR_RED, ON_CLICK_EVENT_ID);
    auto stackSpanMoveCompare = createChildNode(nodeAPI, COLOR_PURPLE, ON_CLICK_EVENT_ID);
    auto stackSpanOffset = createChildNode(nodeAPI, COLOR_RED, ON_CLICK_EVENT_ID_OFFSET);

    ArkUI_AttributeItem id_item = {};
    id_item.string = "OnAreaChangeTestStackSize";
    nodeAPI->setAttribute(stackSpanSize, NODE_ID, &id_item);
    id_item.string = "OnAreaChangeTestStackMove";
    nodeAPI->setAttribute(stackSpanMove, NODE_ID, &id_item);
    id_item.string = "OnAreaChangeTestStackMoveCompare";
    nodeAPI->setAttribute(stackSpanMoveCompare, NODE_ID, &id_item);
    id_item.string = "OnAreaChangeTestStackOffset";
    nodeAPI->setAttribute(stackSpanOffset, NODE_ID, &id_item);

    nodeAPI->addChild(rowSize, stackSpanSize);
    nodeAPI->addChild(rowMove, stackSpanMove);
