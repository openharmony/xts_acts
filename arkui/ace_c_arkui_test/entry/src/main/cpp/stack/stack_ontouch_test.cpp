...
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "StackOnTouchTest", "OnEventReceive");
    if (event == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "StackOnTouchTest", "OnEventReceive: event is null");
        return;
    }

    int32_t eventId = OH_ArkUI_NodeEvent_GetTargetId(event);
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "StackOnTouchTest", "OnEventReceive eventId: %{public}d",
                 eventId);
...
napi_value StackOnTouchTest::CreateNativeNode(napi_env env, napi_callback_info info)
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "StackOnTouchTest", "CreateNativeNode");
    ...
    if ((env == nullptr) || (info == nullptr)) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "StackOnTouchTest", "GetContext env or info is null");
        return nullptr;
    }
    ...
    auto stackEnabled = createChildNode(nodeAPI, true);
    auto stackDisabled = createChildNode(nodeAPI, false);

    ArkUI_AttributeItem id_item = {};
    id_item.string = "OnTouchTest";
    nodeAPI->setAttribute(stackEnabled, NODE_ID, &id_item);
    id_item.string = "OnTouchTestDisable";
    nodeAPI->setAttribute(stackDisabled, NODE_ID, &id_item);

    nodeAPI->addChild(rowFirst, stackEnabled);
    nodeAPI->addChild(rowSecond, stackDisabled);
    ...
    if (OH_NativeXComponent_AttachNativeRootNode(PluginManager::GetInstance()->GetNativeXComponent(id), column) ==
        INVALID_PARAM) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "StackOnTouchTest",
