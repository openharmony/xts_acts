...
static napi_value TestCheckboxSelectedColor001(napi_env env, napi_callback_info info)
{
    ...
    auto ret = nodeAPI->setAttribute(flex, NODE_CHECKBOX_SELECT_COLOR, &value_item);
    ASSERT_EQ(ret, SUCCESS);
    ASSERT_EQ(nodeAPI->getAttribute(flex, NODE_CHECKBOX_SELECT_COLOR)->value[PARAM_0].u32, COLOR_RED);
    NAPI_END;
}
...
static napi_value TestCheckboxSelectedColor002(napi_env env, napi_callback_info info)
{
    ...
    auto ret = nodeAPI->setAttribute(flex, NODE_CHECKBOX_SELECT_COLOR, &value_item);
    ASSERT_EQ(ret, SUCCESS);
    ASSERT_EQ(nodeAPI->getAttribute(flex, NODE_CHECKBOX_SELECT_COLOR)->value[PARAM_0].u32, selectedColor);
    NAPI_END;
}
