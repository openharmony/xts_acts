 ...
 */
#include "swiper_onGestureSwipe_test.h"
...
napi_value SwiperOnGestureSwipeTest::CreateNativeNode(napi_env env, napi_callback_info info)
{
    OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "SwiperOnGestureSwipeTest", "CreateNativeNode");
    ...
    auto swiper = CreateSwiperNode("SwiperOnGestureSwipe");
    swiper->RegisterOnGesture([swiper](ArkUI_NodeEvent *event) {
        ArkUI_NodeComponentEvent *result = OH_ArkUI_NodeEvent_GetNodeComponentEvent(event);
        if (result->data[PARAM_0].i32 == PARAM_2 && result->data[PARAM_1].f32 < PARAM_0) {
    ...
    if (OH_NativeXComponent_AttachNativeRootNode(PluginManager::GetInstance()->GetNativeXComponent(id), column) ==
        INVALID_PARAM) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "SwiperOnGestureSwipeTest",
