...
static napi_value TestTextMinFontSize001(napi_env env, napi_callback_info info)
{
    NAPI_START(text, ARKUI_NODE_TEXT);
    float minFontSizeValue = 20;
...
static napi_value TestTextMinFontSize002(napi_env env, napi_callback_info info)
{
    NAPI_START(text, ARKUI_NODE_TEXT);
    float minFontSizeValue = 0;
    ArkUI_NumberValue value[] = {{.f32 = minFontSizeValue}};
    ArkUI_AttributeItem valueItem = {value, sizeof(value) / sizeof(ArkUI_NumberValue)};
    auto ret = nodeAPI->setAttribute(text, NODE_TEXT_MIN_FONT_SIZE, &valueItem);
    ASSERT_EQ(ret, SUCCESS);
