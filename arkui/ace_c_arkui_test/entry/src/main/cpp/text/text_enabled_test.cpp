...
static auto createChildNode(ArkUI_NativeNodeAPI_1 *nodeAPI, int enabled)
{
    ...
    // set width
    ArkUI_NumberValue width_value[] = {{.f32 = TEXT_WIDTH_200}};
    ArkUI_AttributeItem width_item = {width_value, sizeof(width_value) / sizeof(ArkUI_NumberValue)};
    nodeAPI->setAttribute(nodeHandle, NODE_WIDTH, &width_item);

    // set height
    ArkUI_NumberValue height_value[] = {{.f32 = TEXT_HEIGHT_100}};
...
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "TextEnabledTest", "OnEventReceive");
    if (event == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "TextEnabledTest", "OnEventReceive: event is null");
        return;
    }
...
napi_value TextEnabledTest::CreateNativeNode(napi_env env, napi_callback_info info)
{
    OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "TextEnabledTest", "CreateNativeNode");
    ...
    if ((env == nullptr) || (info == nullptr)) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "TextEnabledTest", "GetContext env or info is null");
        return nullptr;
    }
    ...
    // first text
    auto text = createChildNode(nodeAPI, PARAM_1);

    // second text
    auto textDisabled = createChildNode(nodeAPI, PARAM_0);
    ...
    id_second_item.string = "OnClickDisabledText";
    nodeAPI->setAttribute(textDisabled, NODE_ID, &id_second_item);
    ...
    if (OH_NativeXComponent_AttachNativeRootNode(PluginManager::GetInstance()->GetNativeXComponent(id), column) ==
        INVALID_PARAM) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "TextEnabledTest",
