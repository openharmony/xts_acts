...
static napi_value TestTextTextCase002(napi_env env, napi_callback_info info)
{
    ...
    ASSERT_EQ(nodeAPI->getAttribute(text, NODE_TEXT_CASE)->value->i32, ARKUI_TEXT_CASE_LOWER);
...
static napi_value TestTextTextCase003(napi_env env, napi_callback_info info)
{
    ...
    ArkUI_AttributeItem valueItem = {value, sizeof(value) / sizeof(ArkUI_NumberValue)};
    auto ret = nodeAPI->setAttribute(text, NODE_TEXT_CASE, &valueItem);
    ASSERT_EQ(nodeAPI->getAttribute(text, NODE_TEXT_CASE)->value->i32, ARKUI_TEXT_CASE_UPPER);
    ASSERT_EQ(ret, SUCCESS);
    NAPI_END;
}
...
static napi_value TestTextTextCase004(napi_env env, napi_callback_info info)
{
    ...
    if (nodeAPI->getAttribute(text, NODE_BACKGROUND_IMAGE) != nullptr) {
        ASSERT_NE(nodeAPI->getAttribute(text, NODE_TEXT_CASE)->value->i32, exception);
    };
