...
static void OnEventReceive(ArkUI_NodeEvent *event)
{
    ...
    // if (type != -1 && pressedKeys[0] ==  && count == 1) {
    // if (type != 401) {
...
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "KeyEventGetPressedKeys", "CreateNativeNode");
    ...
    if ((env == nullptr) || (info == nullptr)) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "KeyEventGetPressedKeys", "GetContext env or info is null");
        return nullptr;
    }
    ...
    // id_item.string = "KeyEventGetPressedKeys";
    ...
    if (OH_NativeXComponent_AttachNativeRootNode(PluginManager::GetInstance()->GetNativeXComponent(id), column) ==
         INVALID_PARAM) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "KeyEventGetPressedKeys",
