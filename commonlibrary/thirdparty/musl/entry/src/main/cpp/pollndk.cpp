...
static napi_value Ppoll(napi_env env, napi_callback_info info)
{
    ...
    ret = ppoll(pollfds, 1, &timeout, nullptr);
    close(fd);
...
static napi_value PollChk(napi_env env, napi_callback_info info)
{
    ...
    if (pid == -1) {
        ...
        int ts = 1000;
        poll_chk = __poll_chk(buf, 1, ts, buf[0].fd);
        char buff;
    ...
    napi_value result = nullptr;
    napi_create_int32(env, poll_chk, &result);
    return result;
}
...
static napi_value PpollChk(napi_env env, napi_callback_info info)
{
    ...
    if (pid == -1) {
        ...
        poll_chk = __ppoll_chk(buf, PARAM_1, &ts, nullptr, buf[0].fd);
        char buff;
        while (read(fd[0], &buff, PARAM_1) > 0)
            ;
        close(fd[0]);
    }
