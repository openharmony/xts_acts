...
JSVM_Value GenerateParentClass(JSVM_Env env)
{
    ...
    JSVM_CALL(OH_JSVM_DefineClassWithOptions(env, "parentClass",
        JSVM_AUTO_LENGTH, &parentClassConstructor, JSVM_PARENT_CLASS_DES_COUNT, des,
...
JSVM_Value GenerateSubClass(JSVM_Env env, JSVM_Value parentClass)
{
    ...
    JSVM_CALL(OH_JSVM_DefineClassWithOptions(env, "subClass", JSVM_AUTO_LENGTH, &subClassConstructor,
        0, nullptr, parentClass, JSVM_DEFINE_CLASS_OPTIONS_COUNT, subOptions, &subClass));
    return subClass;
}
