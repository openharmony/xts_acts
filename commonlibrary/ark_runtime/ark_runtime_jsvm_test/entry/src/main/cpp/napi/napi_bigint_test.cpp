...
[[maybe_unused]] JSVM_Value TestCreateAndGetBigIntProgramCase01(JSVM_Env env, JSVM_CallbackInfo info)
{
    ...
    int64_t resultValue = 0;
    status = OH_JSVM_GetValueBigintInt64(env, result, &resultValue, &flag);
    if(status != JSVM_OK || !flag || resultValue != -10){
    ...
    if(retWordCount != 1 || wordsOut[0] != 10){
...
[[maybe_unused]] JSVM_Value TestCreateAndGetBigIntProgramCase02(JSVM_Env env, JSVM_CallbackInfo info)
{
    ...
    if(status != JSVM_OK || !flag1 || resultValue1 != 10){
    ...
    if(retWordCount != 1 || wordsOut[0] != 10){
        OH_JSVM_ThrowError(env, nullptr, "TestCreateAndGetBigIntProgramCase02:GetValueBigintWords retWordCount Failed.");
        return nullptr;
    }
...
[[maybe_unused]] JSVM_Value TestCreateAndGetBigIntProgramCase03(JSVM_Env env, JSVM_CallbackInfo info)
{
    ...
    if(status != JSVM_OK){
        OH_JSVM_ThrowError(env, nullptr, "TestCreateAndGetBigIntProgramCase03:OH_JSVM_CreateBigintWords Failed.");
        return nullptr;
    }
    ...
    if(retWordCount != 2 || wordsOut[0] != 4){
