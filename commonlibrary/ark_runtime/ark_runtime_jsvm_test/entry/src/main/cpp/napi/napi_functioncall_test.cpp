...
[[maybe_unused]] JSVM_Value TestCallFunction03(JSVM_Env env, JSVM_CallbackInfo info)
{
    ...
    JSVM_Status status = OH_JSVM_CoerceToString(env, argv[0], &js_string);
    if (status != JSVM_OK){
        OH_JSVM_ThrowError(env, nullptr, "TestCallFunction03:OH_JSVM_CoerceToString status Failed.");
        return nullptr;
    }
    size_t length = 0;
    status = OH_JSVM_GetValueStringUtf8(env, js_string, NULL, 0, &length);
    if (status != JSVM_OK){
    ...
    size_t copyLength = 0;
    status = OH_JSVM_GetValueStringUtf8(env, js_string, buffer, capacity, &copyLength);
    if (status != JSVM_OK){
        OH_JSVM_ThrowError(env, nullptr, "TestCallFunction03:OH_JSVM_GetValueStringUtf8 status Failed.");
        return nullptr;
    }
    JSVM_Value func = nullptr;
    status = OH_JSVM_CreateFunctionWithScript(env, buffer, JSVM_AUTO_LENGTH, arrayLen,
				                        0, nullptr, &func);
    if (status != JSVM_INVALID_ARG){
...
[[maybe_unused]] JSVM_Value TestCallFunction09(JSVM_Env env, JSVM_CallbackInfo info)
{
    ...
    JSVM_Value run_result;
    JSVM_Script script;
    JSVM_Value js_function_str;
    JSVM_Status status = OH_JSVM_CreateStringUtf8(env, js_function, JSVM_AUTO_LENGTH, &js_function_str);
    if(status != JSVM_OK) {
        OH_JSVM_ThrowError(env, nullptr, "TestCallFunction09:OH_JSVM_CreateStringUtf8 status abnormal.");
        return nullptr;
    }
    status = OH_JSVM_CompileScript(env, js_function_str, nullptr, 0, false, nullptr, &script);
    if(status != JSVM_OK) {
        OH_JSVM_ThrowError(env, nullptr, "TestCallFunction09:OH_JSVM_CompileScript status abnormal.");
        return nullptr;
    }
    status = OH_JSVM_RunScript(env, script, &run_result);
    if(status != JSVM_OK) {
    ...
    // call function
    JSVM_Value result;
    status = OH_JSVM_CallFunction(env, global, add_func, 2, argv, &result);
    if(status != JSVM_OK) {
