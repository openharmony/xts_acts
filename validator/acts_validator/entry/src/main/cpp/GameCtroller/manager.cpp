...
napi_value Manager::CreateNativeNode(napi_env env, napi_callback_info info) {
    ...
    if (nodeAPI != nullptr) {
        ...
        if (nodeAPI->createNode != nullptr && nodeAPI->addChild != nullptr) {
            ...
            ArkUI_NumberValue button_heightValue1[] = {100};
            ...
            ArkUI_AttributeItem LABEL_Item4 = {.string = "FOCUS_AXIS_TEST"};
            ...
            nodeAPI->registerNodeEvent(button, NODE_ON_FOCUS_AXIS, 0 ,nullptr);
            nodeAPI->registerNodeEvent(button, NODE_ON_CLICK, 0 ,nullptr);
            nodeAPI->registerNodeEvent(button, NODE_ON_KEY_EVENT, 0 ,nullptr);
            // button5
            auto button5 = nodeAPI->createNode(ARKUI_NODE_BUTTON);
            ArkUI_AttributeItem LABEL_Item5 = {.string = "冒泡"};
            ...
            nodeAPI->setAttribute(button5, NODE_BACKGROUND_COLOR, &button_color1Item2);
            ...
            nodeAPI->setAttribute(text1, NODE_TEXT_CONTENT, &contentItem);
             ...
             nodeAPI->addChild(innerColumn, text1);
                 
            nodeAPI->registerNodeEvent(button5, NODE_ON_FOCUS_AXIS, 1 ,nullptr);;
            nodeAPI->registerNodeEvent(button5, NODE_ON_KEY_EVENT, 1 ,nullptr);
            // register event
            nodeAPI->addNodeEventReceiver(button, [](ArkUI_NodeEvent *event) {
                auto eventType = OH_ArkUI_NodeEvent_GetEventType(event);
                auto targetId = OH_ArkUI_NodeEvent_GetEventType(event);
                switch (eventType) {
                    case NODE_ON_FOCUS_AXIS:{
                       auto *uiInputEvent = OH_ArkUI_NodeEvent_GetInputEvent(event);
                       auto callback = [](ArkUI_UIInputEvent *event){
                            auto absXValue = OH_ArkUI_FocusAxisEvent_GetAxisValue(event, UI_FOCUS_AXIS_EVENT_ABS_X);
                            auto absYValue = OH_ArkUI_FocusAxisEvent_GetAxisValue(event, UI_FOCUS_AXIS_EVENT_ABS_Y);
                            auto absZValue = OH_ArkUI_FocusAxisEvent_GetAxisValue(event, UI_FOCUS_AXIS_EVENT_ABS_Z);
                            auto absRzValue = OH_ArkUI_FocusAxisEvent_GetAxisValue(event, UI_FOCUS_AXIS_EVENT_ABS_RZ);
                            auto absGasValue = OH_ArkUI_FocusAxisEvent_GetAxisValue(event, UI_FOCUS_AXIS_EVENT_ABS_GAS);
                            auto absBrakeValue = OH_ArkUI_FocusAxisEvent_GetAxisValue(event, UI_FOCUS_AXIS_EVENT_ABS_BRAKE);
                            auto absHat0XValue = OH_ArkUI_FocusAxisEvent_GetAxisValue(event, UI_FOCUS_AXIS_EVENT_ABS_HAT0X);
                            auto absHat0YValue = OH_ArkUI_FocusAxisEvent_GetAxisValue(event, UI_FOCUS_AXIS_EVENT_ABS_HAT0Y);
                            OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "Manager", "button0  absXValue == %{public}f, absYValue == %{public}f"
                            "absZValue == %{public}f absRzValue ==%{public}f absGasValue == %{public}f absBrakeValue == %{public}f bsHat0XValue == %{public}f"
                            "bsHat0YValue == %{public}f", absXValue, absYValue, absZValue, absRzValue, absGasValue, absBrakeValue, absHat0XValue, absHat0YValue);
                           OH_ArkUI_FocusAxisEvent_SetStopPropagation(event, false);
                            std::string absx = "absX: ";
                            std::string absy = "; absY: ";
                            std::string absz = "; absZ: ";
                            std::string absrz = "; absRz: ";
                            std::string absgas = "; absGas: ";
                            std::string absbreak = "; absBrake: ";
                            std::string absHat0X = "absHat0X: ";
                            std::string absHat0Y = "; absHat0Y";
                            std::string str =  absx + std::to_string(absXValue) + absy + std::to_string(absYValue) + absz +  std::to_string(absZValue) + absrz +  std::to_string(absRzValue ) + absgas +  std::to_string(absGasValue) + absbreak +  std::to_string(absBrakeValue) + absHat0X +  std::to_string(absHat0XValue) + absHat0Y +  std::to_string(absHat0YValue);
                            ArkUI_AttributeItem contentItem = {.string =  str.c_str()};
                            nodeAPI->setAttribute(text1, NODE_TEXT_CONTENT, &contentItem); 
                       };
                       callback(uiInputEvent);  
                       break;
                    }
                    default: {
                        break;
            ...
            nodeAPI->addNodeEventReceiver(button5, [](ArkUI_NodeEvent *event) {
                ...
                switch (eventType) {
                    case NODE_ON_FOCUS_AXIS:{
                        if (targetId == 1) {
                            auto *uiInputEvent = OH_ArkUI_NodeEvent_GetInputEvent(event);
                            OH_ArkUI_FocusAxisEvent_SetStopPropagation(uiInputEvent, true);
                            auto callback = [](ArkUI_UIInputEvent *event){
                            auto absXValue = OH_ArkUI_FocusAxisEvent_GetAxisValue(event, UI_FOCUS_AXIS_EVENT_ABS_X);
                            auto absYValue = OH_ArkUI_FocusAxisEvent_GetAxisValue(event, UI_FOCUS_AXIS_EVENT_ABS_Y);
                            auto absZValue = OH_ArkUI_FocusAxisEvent_GetAxisValue(event, UI_FOCUS_AXIS_EVENT_ABS_Z);
                            ...
                            auto absHat0XValue = OH_ArkUI_FocusAxisEvent_GetAxisValue(event, UI_FOCUS_AXIS_EVENT_ABS_HAT0X);
                            auto absHat0YValue = OH_ArkUI_FocusAxisEvent_GetAxisValue(event, UI_FOCUS_AXIS_EVENT_ABS_HAT0Y);
                            OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "Manager", "button5  absXValue == %{public}f, absYValue == %{public}f"
                            "absZValue == %{public}f absRzValue ==%{public}f absGasValue == %{public}f absBrakeValue == %{public}f bsHat0XValue == %{public}f"
                            "bsHat0YValue == %{public}f", absXValue, absYValue, absZValue, absRzValue, absGasValue, absBrakeValue, absHat0XValue, absHat0YValue);
                       };
                            callback(uiInputEvent);
                ...
                switch (eventType) {
                    case NODE_ON_FOCUS_AXIS:{
                       ...
                       auto callback = [](ArkUI_UIInputEvent *event){
                            ...
                            OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "Manager", "innercolumn callback tirgger");
                       };
                       callback(uiInputEvent);  
                       break;
                    }
            ...
            nodeAPI->registerNodeEvent(innerColumn, NODE_ON_FOCUS_AXIS, 0 ,nullptr);
                ...
                switch (eventType) {
                    case NODE_ON_FOCUS_AXIS:{
                       ...
                       auto callback = [](ArkUI_UIInputEvent *event){
                            ...
                            OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "Manager", "stack callback tirgger");
                       };
                       callback(uiInputEvent);  
                       break;
                    }
            ...
            nodeAPI->registerNodeEvent(stack, NODE_ON_FOCUS_AXIS, 0 ,nullptr);
            nodeAPI->registerNodeEvent(stack, NODE_ON_KEY_EVENT, 0 ,nullptr);
